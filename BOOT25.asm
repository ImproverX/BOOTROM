;#define BOOT2K 1
; Раскомментировать для сборки на 2048 байт. Убирается:
; - Бейсик
; - Монитор
; - Загрузка из РС через порты ПУ-LPT
; - FM9
; - Реанимация 0 блока монитора
; - Загрузка модуля выгрузки данных через ПУ (в мониторе)
; Загрузка с флоппи переключается на клавишу F3
; Вместо запуска FM9 по кнопке "влево-вверх" запускается программа с адреса 0800h (второй чип ПЗУ)
;
;#define NoBEEP 1
; Раскомментировать для отключения сигнала при запуске
;
...
M_VAR1	.EQU    0DED0h	; DB/DW тут хранится указатель стека (КД) / начало загрузки / сколько секторов НЖМД читать, номер сектора НЖМД
...
#else
        LXI     B,40H   ;
RTIM:   OUT     08H     ;реж-4,ст.байт
        SUB     C       ;счет каналов 2,1,0
        JNC     RTIM    ;переход если не все каналы таймера выключены
#endif
	LXI  SP,0E000h
	LXI  H, 0C13Eh
...
L_0039:	LXI  SP,00000h	; стартовый адрес
	MVI  B, 080h	; счётчик
L_003E:	MOV  L, C	;LXI  H, 00000h	; чем заполнять
#ifndef BOOT2K
	XRA  A
	OUT	010h
	OUT	011h	; отключаем КД (не обязательно для реала)
#endif
L_CLRM:	PUSH H
	DCR  C
	JNZ     L_CLRM
...
;	MOV  B, A	; B = 00h
	...
	OUT     008h	; <- реж-4,ст.байт-(одновибратор) -- отключение писка
#endif
...
L_0059:	MOV  H, B
	...
	LDA     S_KBD
#ifdef BOOT2K
	JZ	L_042B	; (УС нажата) >> магнитофон
#else
...
L_0365:	LXI  H, L_0755	; дискетка	; << загрузка с флоповода
	...
	PUSH D		; сохраняем количество блоков для таблицы
;	MOV  B, A
...
L_LOOP:	IN      057h	; регистр статуса
	ANI     0C0h	; 1100 0000
	CPI     040h
	RZ		; > выход из ПП (A = 040h, Z=1)
	DCX  B
	MOV  A, B
	ORA  C
	JNZ     L_LOOP	; цикл на 65536 попыток
L_HDE:	INR  A
	RET		; >>>> долго ждём, ошибка (A = 1, Z=0)
;
; проверка наличия НЖМД, вых. Z = 1 -- всё ок.
L_CHDD:	IN      057h	; регистр статуса НЖМД
	INR  A		;
	ANI     0C0h	; 1100 0000
	JZ	L_HDE	; выход с ошибкой если А = FFh или A < 3Fh -- проверка на отсутствие НЖМД
...
L_CHBT:	LXI  H, B_DRV	; <<<<< ПП проверки загрузочного сектора (НГМД+НЖМД)
	LXI  D, M_BT	; куда будет скопирован загрузочный сектор
	MVI  C, 01Fh
	MVI  A, 066h
L_CBT1:	MOV  B, A
	MOV  A, M
	...
	RET		; выход (Z=1)
;
; ПП установки параметров загрузки (НГМД+НЖМД)
L_SETB:	LXI  D, 0FF80h	; (-80h)
	...
	ADD  H
	MOV  E, A	; E = конечный адрес загрузки
	RET
;
...
L_067B:	LXI  H, L_078E	; ссылка на картинку с диском
	...
	MOV  A, H
	CMP  E
	JNC	L_DDN	; >> загружено всё
	CALL    L_RES	; сброс и ожидание готовности НЖМД
	CALL    L_SETB	; чтение параметров загрузки
	CALL    L_RHD2	; второй вариант чтения НЖМД, если не работает потоковое чтение
...
L_FBL1:	CALL    L_015A	; >> заполнение блока,  << D=количество блоков
	...
	RET
;
L_RHDD:	MOV  A, C	; чтение загрузочного сектора потоком
	STA	M_VAR1	; сколько секторов читать
	MVI  A, 002h	; сектор №2
L_RHD0:	OUT     053h	; LBA [7..0]
	XRA  A
	OUT     054h	; LBA [15..8]
	OUT     055h	; LBA [23..16]
	MVI  A, 0E0h	; 1110 0000
	OUT     056h	; режим и LBA[27..24]
	LDA	M_VAR1	; сколько секторов читать
...
L_RLP:	IN      057h	; регистр статуса
	...
	JMP     L_RLP
;
L_RHD2:	MVI  A, 001h	; второй метод, посекторное чтение
	STA	M_VAR1	; сколько секторов читать
	MOV  E,C	; счётчик
	INR  A		;MVI  A, 002h	; сектор №2
L_RHD3:	STA	M_VAR1+1	; считываемый сектор
	CALL	L_RHD0
	DCR  E
	RZ		; >> всё прочитано
	LDA	M_VAR1+1	; сектор
	INR  A
	JMP	L_RHD3	; цикл по секторам НЖМД
;
;==========================================================
L_06CB:	.db 008h	; высота
	.db 0F3h	; - |■■■■  ■■| ВЕКТОР-06Ц.24
	.db 0DBh	; - |■■ ■■ ■■|
	.db 0DBh	; - |■■ ■■ ■■|
	.db 0DBh	; - |■■ ■■ ■■|
	.db 0F3h	; - |■■■■  ■■|
...
#else
	.org 007FFh
	.db 000h	; контрольная сумма (offset 07FFh)
#endif
